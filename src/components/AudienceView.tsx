...
import { ThumbsUp, Send, User, CheckCircle2, MessageSquare, Filter, Clock, Flame, UserCheck, Sparkles, AlertCircle } from 'lucide-react';
...
export const AudienceView: React.FC<AudienceViewProps> = ({
  ...
  const publicQuestions = questions.filter(q => 
    q.status === 'approved' || q.status === 'pushed' || q.status === 'answering' || q.status === 'answered' || mySubmittedIds.includes(q.id)
  );
  ...
  const getStatusBadge = (status: Question['status']) => {
    switch (status) {
      case 'answering':
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-rose-500 text-white animate-pulse">🎙️ ANSWERING LIVE NOW</span>;
      case 'pushed':
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200">✨ Pushed to Panel</span>;
      case 'approved':
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">Approved</span>;
      case 'answered':
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl p-6 shadow-xl border border-indigo-900/40 relative overflow-hidden">
        <div className="relative z-10 space-y-2">
          <div className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-500/20 text-indigo-300 border border-indigo-400/30">
            Event Join Code: <span className="font-mono font-bold ml-1 text-white">{conferenceEvent.joinCode}</span>
          </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-slate-900 text-lg">Ask the Panel a Question</h3>
          </div>
        ...
        {submittedSuccess && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center space-x-3 text-emerald-800 text-sm animate-fadeIn">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-700">Your question is now with the backend team for live panel selection.</p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <textarea
              ...
              placeholder="What question would you like answered live by the panel members?"
              maxLength={300}
              rows={3}
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-200">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-slate-900 text-lg">Audience Questions</h3>
        ...
        {filteredQuestions.length === 0 ? (
          <div className="bg-white rounded-2xl p-10 text-center border border-slate-200/80 space-y-3">
            ...
            <h4 className="text-slate-700 font-semibold">No questions in this feed yet</h4>
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              Be the first to ask a question using the form above! Submitted questions will appear here once reviewed.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    >
                      <ThumbsUp className={`w-4 h-4 mb-0.5 ${isUpvotedByMe ? 'fill-indigo-600' : ''}`} />
